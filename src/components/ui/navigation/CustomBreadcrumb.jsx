...
const CustomBreadcrumb = ({
  ...
  return (
    <div className={variant === 'minimal' ? 'mb-4' : 'mb-6'}>
      ...
      <div
        className={`${getContainerClasses()} ${getAnimationClasses()} sm:text-sm text-xs sm:p-4 p-2 overflow-x-auto`}
        style={{ WebkitOverflowScrolling: 'touch' }}
      >
        {' '}
        {/* Responsive font and padding, scrollable on mobile */}
        <nav className='flex items-center space-x-2' aria-label='Breadcrumb'>
          <ol
            className={`flex items-center space-x-2 sm:text-sm text-xs whitespace-nowrap`}
            style={{ scrollbarWidth: 'none' }}
          >
            {' '}
            {/* Responsive font size, prevent wrap */}
            {/* Home Icon */}
            {showHome && (
              <li className='flex items-center'>
                ...
                >
                  <MdHome
                    className={`${variant === 'compact' ? 'w-4 h-4' : 'w-5 h-5'} mr-1 group-hover:text-blue-600 transition-colors duration-300 sm:w-5 sm:h-5 w-4 h-4`}
                  />
                  <span
                    className={`${variant === 'minimal' ? 'inline' : 'hidden sm:inline'} font-medium group-hover:text-blue-600 sm:text-base text-xs`}
                  >
                ...
                {breadcrumbLinks.length > 0 && (
                  <MdChevronRight
                    className={`${variant === 'compact' ? 'w-3 h-3 mx-1' : 'w-4 h-4 mx-2'} text-gray-400 sm:w-4 sm:h-4 w-3 h-3 sm:mx-2 mx-1`}
                  />
                )}
              </li>
            )}
            ...
            {title && (
              <li className='flex items-center'>
                <div className='relative group'>
                  ...
                  <span
                    className={`relative sm:px-4 px-2 sm:py-2 py-1 font-bold sm:text-base text-xs bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent ${variant !== 'minimal' ? 'glass rounded-xl border border-white/20 backdrop-blur-sm' : ''}`}
                  >
                    {title}
                  </span>
                </div>
