...
const MonthlyFees = ({ classId, onFeeChange }) => {
  ...
  useEffect(() => {
    if (classId) fetchFeesData(classId);
  }, [classId]);
  ...
  const fetchFeesData = async classId => {
    setLoading(true);
    try {
      ...
      const response = await fetch(`${baseUrl}/api/Fees/GetMonthlyFees`, {
        method: 'POST',
  ...
  const handleChipClick = feeHead => {
    const isAllSelected =
      (selectedMonths[feeHead] || []).length ===
      (groupedFees[feeHead] || []).length;
            ...
            return (
              ...
              >
                <Box sx={{ position: 'relative' }}>
                  <Chip
                    ...
                    sx={{
                      ...
                      borderRadius: 10,
                      backgroundColor:
                        selected.length > 0 ? 'primary.main' : '#eee',
                      '&:hover': {
                        backgroundColor:
                          selected.length > 0 ? 'primary.dark' : '#ddd',
                      },
                    }}
                  />
                ...
                {openDropdown === feeHead && (
                  <FormControl
                    fullWidth
                    sx={{ mt: 1, position: 'relative', zIndex: 1300 }}
                  >
                    <Autocomplete
                      multiple
