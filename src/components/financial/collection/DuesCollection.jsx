...
const DuesCollection = () => {
  ...
  const fetchReceiptNumber = async admissionId => {
    try {
      // Try dues billing endpoint first
      let data;
      try {
        data = await api.dues.getBillingReceiptNumber(admissionId);
      } catch (duesError) {
        // If dues endpoint fails (404, etc.), try the monthly billing endpoint as fallback
        console.warn(
          'Dues billing endpoint not available, trying monthly billing fallback:',
          duesError.message,
        );
      ...
      console.error('Failed to fetch receipt number from both endpoints:', err);
      setReceiptNumber('N/A');
    }
  };
  ...
  return (
    ...
    >
              ...
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.25 }}>
                ...
                <Box
                  sx={{
                    ...
                    transition:
                      'max-width 300ms ease, opacity 250ms ease, transform 250ms ease',
                    overflow: 'hidden',
                  }}
                >
                  <TextField
                    ...
                    placeholder={
                      isSearchOpen
                        ? "Search by name, admission no, or father's name"
                        : 'Search...'
                    }
                    value={searchText}
      ...
      >
        {' '}
        <Box sx={{ maxWidth: 1200, mx: 'auto', px: { xs: 2, sm: 3 }, pb: 3 }}>
          {' '}
          {/* Search Results */}
          {searchText.trim() && (
            <Card
                        ...
                        <List dense disablePadding sx={{ pb: 2 }}>
                          {(() => {
                            ...
                            return paginatedStudents.map(student => {
                              const pending = getPendingFromDues(
                              ...
                              const classLabel = [
                                ...
                                .filter(Boolean)
                                .join('-');
                              const rollLabel = Number.isFinite(
                                Number(student.rollNo),
                              )
                                ? `Roll ${student.rollNo}`
                                : 'Roll -';
                              const admissionLabel = student.admissionNo
                                  ...
                                  >
                                    ...
                                    <Box
                                      sx={{
                                        ...
                                        borderRadius: 1.2,
                                        background:
                                          'linear-gradient(90deg, rgba(16,185,129,0.05), rgba(16,185,129,0.02))',
                                        minWidth: 0,
                                        borderTop: {
                                    ...
                                    <Box
                                      sx={{
                                        ...
                                        borderRadius: 1.2,
                                        background:
                                          'linear-gradient(90deg, rgba(239,68,68,0.05), rgba(239,68,68,0.02))',
                                        borderTop: {
                                          xs: '1px solid rgba(0,0,0,0.08)',
                                          sm: 'none',
                                        },
